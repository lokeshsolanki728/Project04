...
public class UserModel extends BaseModel {
        ...
        try {
          conn = JDBCDataSource.getConnection();
           ...
           try(PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM ST_USER WHERE ID = ?")){
               pstmt.setLong(1, pk); try(ResultSet rs = pstmt.executeQuery()){ if (rs.next()) {
                        populateBean(rs, dto);
                     }
            }
        } catch (SQLException e) {
    ...
    public List getRoleList() throws ApplicationException { // Ignoring UserBean parameter
        ...
        try {
            conn = JDBCDataSource.getConnection();
           try (PreparedStatement pstmt = conn.prepareStatement("SELECT ID, NAME FROM ST_ROLE"); ResultSet rs = pstmt.executeQuery()){ while (rs.next()) {
    ...
    public Long registerUser(UserDTO dto) throws DuplicateRecordException, ApplicationException {
        ...
        try {
           ...
           try(PreparedStatement pstmt = conn.prepareStatement("INSERT INTO ST_USER VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)")){
                ...
                pstmt.setString(4, dto.getLogin());
                pstmt.setString(5, dto.getPassword());
                pstmt.setDate(6, DataUtility.getDate(dto.getDob()));
                pstmt.setString(7, dto.getMobileNo());
                pstmt.setLong(8, dto.getRoleId());pstmt.setString(9, dto.getGender());
          ...
          connection.commit();
         } catch (SQLException e) { log.error("Database Exception in : registerUser", e);
            JDBCDataSource.trnRollback(conn);
