				...
				<table class="search-table w-100">
					<tr>
						<td class="text-center"><label for="name">Course
							...
							placeholder="Enter Course Name" class="form-control-inline"
							value="${param.name}"> <input type="submit"
					...
					<tbody>
						<c:forEach var="course" items="${list}" varStatus="loop">
							<tr class="text-center table-row">
								...
								<td><a href="${ctx}${ORSView.COURSE_CTL}?id=${course.id}" class="btn btn-link">Edit</a></td>
							</tr>
						</c:forEach>
					</tbody> 
				</table>
				<table class="w-100">
					<tr>
						<td class="text-left"><input type="submit" name="operation"
							value="${CourseListCtl.OP_PREVIOUS}" class="btn btn-secondary"
							${pageNo == 1 ? 'disabled' : ''}>
						</td>
						<td><input type="submit" name="operation" value="${CourseListCtl.OP_DELETE}" class="btn btn-danger"></td>
						<td><input type="submit" name="operation" value="${CourseListCtl.OP_NEW}" class="btn btn-success"></td>
						<td class="text-right"><input type="submit" name="operation" value="${CourseListCtl.OP_NEXT}"
							class="btn btn-primary" ${list.size() < pageSize || nextPage == 0 ? 'disabled' : ''}></td>
					</tr>
				</table>
			</c:if>
